...
{		 
	...
	GPIO_AF_Set(GPIOH,10,2);	//PH10,AF2
  GPIO_AF_Set(GPIOH,11,2);	//PH11,AF2
	GPIO_AF_Set(GPIOH,12,2);	//PH10,AF2
...
//	TIM5->CCER|=1<<1; 		//CC1P=0	下降沿捕获
//	TIM5->CCER|=1<<0; 		//CC1E=0 	允许捕获计数器的值到捕获寄存器中
//	
//	TIM5->CCER|=0<<5; 		//CC2P=0	上升沿捕获
//	TIM5->CCER|=1<<4; 		//CC2E=0 	允许捕获计数器的值到捕获寄存器中
	
	TIM5->CCER|=0<<9; 		//CC3P=0	上升沿捕获
	TIM5->CCER|=1<<8; 		//CC3E=0 	允许捕获计数器的值到捕获寄存器中
...
{
	float Puse=0;
	if(mode) Puse=(float)H_PUSE1/10000.0f;
	else  Puse=(float)H_PUSE2/10000.0f;
	
	sys_print("H_PUSE:%0.6fms\r\n",Puse);
	return Puse;
}
